...
if [[ "$COMPLETION_PROMISE" != "null" ]] && [[ -n "$COMPLETION_PROMISE" ]]; then
  ...
  if [[ -n "$PROMISE_TEXT" ]] && [[ "$PROMISE_TEXT" = "$COMPLETION_PROMISE" ]]; then
    ...
    if [[ -n "${VERIFY_CMD//[[:space:]]/}" ]] && [[ "$VERIFY_CMD" != "null" ]]; then
      ...
      # Missing verification cannot establish completion. Pause rather than
      # forcing another autonomous iteration or recording a successful result.
      PROMISE_REJECTIONS=$((PROMISE_REJECTIONS + 1))
      TEMP_FILE="${RALPH_STATE_FILE}.tmp.$$"
      sed 's/^active:.*/active: false/' "$RALPH_STATE_FILE" | \
        sed "s/^promise_rejections: .*/promise_rejections: $PROMISE_REJECTIONS/" > "$TEMP_FILE"
      mv "$TEMP_FILE" "$RALPH_STATE_FILE"
      jq -nc --argjson iteration "$ITERATION" --argjson rejections "$PROMISE_REJECTIONS" \
        '{iteration:$iteration,status:"promise_rejected",reason:"missing_verifier",rejections:$rejections}' \
        >> .claude/pua-loop-history.jsonl 2>/dev/null || true
      echo "PUA Loop: completion REJECTED — no verify command configured; paused for user review." >&2
      exit 0
    fi
