...
# Archived reference only. Telemetry requires explicit opt-in; missing, malformed,
# or unrecognized settings create no identity and send no request.
is_true "$OFFLINE" && exit 0
is_true "$TELEMETRY" || exit 0
