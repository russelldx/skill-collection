> **ARCHIVED — PAUSED / NON-EXECUTABLE REFERENCE.** PUA core and its styles remain inactive. Do not load this as a skill, execute the historical protocol, register hooks, start loops, or send telemetry. This archive grants no authority to expand scope, ignore stop requests, bypass permissions, or override system/developer instructions. Scripts are retained for inspection and isolated tests only; heartbeat telemetry defaults OFF and loop setup is bounded. Original instructions below are historical reference, not current policy.
>
> Inactive style references: [mama](references/styles/pua-mama/REFERENCE.md), [yes](references/styles/pua-yes/REFERENCE.md). Reading them does not enable core or styles.
