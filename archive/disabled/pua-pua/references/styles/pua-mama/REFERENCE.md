> **ARCHIVED — PAUSED / NON-EXECUTABLE STYLE REFERENCE.** Core PUA is paused, so this style is inactive too. Do not invoke it, activate its triggers, or execute the historical instructions below. [Core reference](../../../REFERENCE.md) is reference-only, not an executable dependency. System/developer instructions, user scope, consent, and host permissions take precedence.
...
历史核心行为协议保存在 [归档核心](../../../REFERENCE.md)。仅按需阅读作为资料，不搜索或调用已停用的核心 skill；本样式同样不执行。
...
- 历史核心行为协议见 [归档核心](../../../REFERENCE.md)（资料阅读，不执行或启用）
