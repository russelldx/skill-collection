> **ARCHIVED — PAUSED / NON-EXECUTABLE REFERENCE.** Do not invoke, register hooks, or start a loop from this document. The historical unlimited/honor-system descriptions below are superseded for the retained scripts: setup defaults to 10 iterations (valid range 1–100), missing verification rejects completion and pauses, and a user may stop at any time. A verifier stored in an editable state file is not an immutable security boundary. Host permissions and system/developer instructions always apply. Core reference: [paused PUA core](../pua-pua/REFERENCE.md); retained script: `../pua-pua/scripts/setup-pua-loop.sh`. Original material follows for reference only.
