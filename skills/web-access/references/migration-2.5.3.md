...
旧写法收到 `400` + 包含本文档路径的错误信息后，按下方对照表改写**当前调用**。发现源站点经验文件或脚本里仍有旧写法时，改写这些文件属于对技能资源的写入：仅在用户明确授权修改这些文件时进行，否则作为待办提案报告，不自动改动源文件。
...
3. **提案更新源文件**：如果旧写法来自 `references/site-patterns/*.md` 中某个站点经验文件或其它脚本，把"修正该文件"作为提案报告给用户（附上具体位置与改法）；只有在用户明确授权写入这些技能资源时才实际修改——否则只改当前调用，并记录待办，避免下次复用同一经验时再踩坑。
4. **保留 URL 原样**：POST body 是不透明字节流，目标 URL 中的 `?`、`&`、`=`、`#`、`%` 等字符**无需 HTTP query 转义**，从 DOM/分享链接抓到什么样就传什么样。注意这只说明 HTTP 编码层面无负担，**不代表把 URL 拼进 shell 命令或脚本是安全的**：不要直接插值未校验的 URL，用参数化、引号包裹等不解析输入的方式传参，并只访问已授权的非敏感目标。
